<div class="card-header border-0 order-header-shadow">
    <h5 class="card-title d-flex justify-content-between">
        <span>{{ translate('messages.top_riders') }}</span>
    </h5>
        ...
        <div class="top--selling">
            @foreach ($top_riders as $key => $item)
                <a class="grid--card" href="{{ route('admin.users.rider.preview', [$item['id']]) }}">
                    ...
                    <div class="ml-auto">
                        <span class="badge badge-soft">{{ translate('messages.rides') }} :
                            {{ $item['driver_trips_count'] }}</span>
                    </div>
                </a>
            @endforeach
        </div>
