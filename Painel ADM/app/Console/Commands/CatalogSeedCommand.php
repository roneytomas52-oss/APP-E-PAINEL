...
class CatalogSeedCommand extends Command
{
    ...
    private function resolveMainCategoryLocalImage(string $moduleType, string $categoryName): ?string
    {
        ...
        $targetFile = "{$targetDir}/{$mapped['file']}";
        $publicBinary = file_get_contents($publicAssetPath);

        if (
            !$disk->exists($targetFile)
            || md5((string)$disk->get($targetFile)) !== md5((string)$publicBinary)
        ) {
            $disk->put($targetFile, $publicBinary);
        }
...
SVG;
        ...
        $current = is_file($filePath) ? (string)file_get_contents($filePath) : '';
        if (md5($current) !== md5($svg)) {
